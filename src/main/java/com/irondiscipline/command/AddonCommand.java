...
import com.irondiscipline.manager.AddonManager;
import com.irondiscipline.manager.AddonManager.AddonDescriptor;
import com.irondiscipline.manager.AddonManager.AddonRegistryEntry;
import com.irondiscipline.manager.AddonManager.InstallResult;
...
public class AddonCommand {
    ...
    private void handleInstall(CommandSender sender, String[] args) {
        ...
        if (target.startsWith("http://") || target.startsWith("https://")) {
            addonManager.installFromUrl(target).thenAccept(result -> sendResult(sender, result));
        } else if (target.contains("/")) {
            addonManager.installFromGitHub(target).thenAccept(result -> sendResult(sender, result));
        } else {
            addonManager.installCertified(target).thenAccept(result -> sendResult(sender, result));
        }
    }
    ...
    private void handleCertified(CommandSender sender) {
        ...
        for (Map.Entry<String, AddonRegistryEntry> entry : certified.entrySet()) {
            ...
            boolean isInstalled = installed.stream().anyMatch(d -> d.id().equalsIgnoreCase(addon.id()));

            String status = isInstalled ? ChatColor.GREEN + "[導入済]" : ChatColor.YELLOW + "[未導入]";
            sender.sendMessage(ChatColor.translateAlternateColorCodes('&',
    ...
    private void handleRemove(CommandSender sender, String[] args) {
        ...
        plugin.getAddonManager().uninstall(addonId).thenAccept(result -> sendResult(sender, result));
    }
